...
bg = PhotoImage(file="G:/TA PDKP/18.png")
workbook = Workbook()
    ...
    def main_screen():
            ...
            def login():
                ...
                if(username == user and password == pas):
                    ...
                    messagebox.showwarning("Gagal", "Maaf anda gagal login")
            
            frameLog = Frame(root, bg='white')
            frameLog.place(rely=0.025,relx=0.5,relheight=0.1,relwidth=0.8,anchor='n')
            label = Label(frameLog, bg='white', text="LOGIN", font=styling)
            label.place(relheight=1,relwidth=1)

            frameUser = Frame(root, bg='white')
            frameUser.place(rely=0.2,relx=0.5,relheight=0.06,relwidth=0.8,anchor='n')
            Userinfo = Label(frameUser, bg='white', text='Username', font=styling2)
            Userinfo.place(relwidth=0.4,relheight=1)
            e1 = Entry(frameUser)
            e1.place(relx=0.4,relheight=1,relwidth=0.6)        
            
            framePW = Frame(root, bg='white')
            framePW.place(rely=0.27,relx=0.5,relheight=0.06,relwidth=0.8,anchor='n')
            PWinfo = Label(framePW, bg='white', text='Password', font=styling2)
            PWinfo.place(relwidth=0.4,relheight=1)
            e2 = Entry(framePW)
            e2.place(relx=0.4,relheight=1,relwidth=0.6)

            frametombol = Frame(root, bg='white')
            frametombol.place(rely=0.675,relx=0.5,relheight=0.06,relwidth=0.3,anchor='n')
            tombol = Button(frametombol, bg='white', text="Login", command=login)
            tombol.place(relx=0.5,relheight=1,relwidth=1,anchor='n')
...
class window2:
    def absensi():
        ...
        def SaveData():
            ...
            informasi['text'] = "Data absen telah disimpan dengan \nNama file: "+str(matkulEntry.get())+"_"+str(tanggalEntry.get())+".xlsx"

        def CreateNewData():
            global informasi, num
            informasi['text'] = 'Silahkan isi data absensi\n klik insert untuk masukkan data dan klik simpan untuk menyimpan data.'
            namaEntry.delete(0, END)
            NIMEntry.delete(0, END)
            jurusanEntry.delete(0, END)
        ...
        frameJudul = Frame(root, bg='yellow')
        frameJudul.place(rely=0.025,relx=0.5,relheight=0.1,relwidth=0.8,anchor='n')
        judul = Label(frameJudul, bg='yellow', text='Absensi Perkuliahan', font=styling)
        ...
        namainfo = Label(frameNama, bg='white', text='Nama Mahasiswa', font=styling2)
        ...
        NIMinfo = Label(frameNIM, bg='white', text='NIM Mahasiswa', font=styling2)
        ...
        informasi = Label(root, bg='yellow', font=styling2, text='Silahkan isi data absensi\n klik insert untuk masukkan data dan klik simpan untuk menyimpan data.')
        ...
        insert = Button(frameButton, bg='limegreen', text='Insert', command=InsertData)
        insert.place(rely=0,relx=0.5,relheight=0.25,relwidth=1,anchor='n')
        save = Button(frameButton, bg='limegreen', text='Simpan', command=SaveData)
        save.place(rely=0.25,relx=0.5,relheight=0.25,relwidth=1,anchor='n')
        createNewData = Button(frameButton, bg='limegreen', text='Buat Baru', command=CreateNewData)
        createNewData.place(rely=0.5,relx=0.5,relheight=0.25,relwidth=1,anchor='n')
        Exit = Button(frameButton, bg='red', text='Keluar', command=root.quit)
